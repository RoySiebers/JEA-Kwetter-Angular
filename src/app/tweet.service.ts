...
const TWEET_URL = "ws://localhost:8080/JEA-Kwetter/socket";
const httpOptions = {
...
export class TweetService {
  ...
  updateTweet(tweet: Tweet): Observable<any> {
    return this.http
      .put(this.tweetsUrl, tweet, httpOptions)
      .pipe(
  ...
  addTweet(tweet: Tweet): Observable<Tweet> {
    ...
    return this.http
      .post<Tweet>(url, tweet, httpOptions)
      .pipe(
  ...
  searchTweets(term: string): Observable<Tweet[]> {
    ...
    return this.http
      .get<Tweet[]>(`${this.tweetsUrl}/allTweetsUsername/${term}`)
      .pipe(
